...
impl Matcher for MatchWhitespace {
    fn try_match (&self, tok: &mut Tokenizer) -> Option<Token> {
        ...
        if found_ws {
            return Some (Token { tok_type: TokenType::WhiteSpace, line: tok.line, col: tok.column})
        }
...
impl Matcher for MatchNumber {
    fn try_match (&self, tok: &mut Tokenizer) -> Option<Token> {
        ...
        if !number.is_empty () {
            return Some (Token { tok_type: TokenType::IntLiteral (number.parse::<u64> ().unwrap ()), line: tok.line, col: tok.column })
        }
